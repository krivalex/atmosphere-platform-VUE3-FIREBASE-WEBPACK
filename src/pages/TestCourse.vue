...
export default {
  ...
  unmounted() {
    ...
    if (this.timer === 0) {
      localStorage.setItem('test_timer', 0);
    }
    this.timer = this.timer + Number(timer_end) - Number(this.timer_start);
    if (Number(localStorage.getItem('test_timer')) >= Number(this.timer) || localStorage.getItem('test_timer') == 0) {
      localStorage.setItem('test_timer', this.timer);
    }
    else {
      localStorage.setItem('test_timer', this.timer)
    }
  ...
  data() {
    return {
      ...
      timer: localStorage.getItem('test_timer') ? localStorage.getItem('test_timer') : 0,
  ...
  methods: {
    ...
    clickEmpthy() {
      if (this.number_of_question < database.length) {
        ...
        const all_courses = [this.python_start, this.python_junior, this.scratch, this.gamedev, this.comp_science]
        const all_courses_names = ["IT-Start", "IT-Adult", "IT-Junior", "Scratch", "GameDev", "Компьютерная грамотность"]
        this.question = "Вам больше всего подходит курс:"
        this.number_of_question = "∞"
        this.result = all_courses_names[all_courses.indexOf(Math.max(...all_courses))]
        console.log(this.question)
